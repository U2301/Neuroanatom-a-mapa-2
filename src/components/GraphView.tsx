...
import { Crosshair, Plus, Minus, Scroll, Zap, Microscope, Brain, Link as LinkIcon, Hand, Pin, Compass, ShieldCheck } from 'lucide-react';
...
export const GraphView: React.FC<GraphViewProps> = () => {
  ...
  const [selectedNode, setSelectedNode] = useState<NeuroNode>(() => {
    return neuroNodes.find(n => n.id === 1) || neuroNodes[0];
  });
  ...
  useEffect(() => {
    ...
    const initNetwork = () => {
      ...
      const formattedNodes = neuroNodes.map(node => {
        ...
        if (node.group === "planimetria") {
          bgColor = "#edf5f0";
          borderColor = "#2b553c";
          fontColor = "#162e21";
        } else if (node.group === "lobulos") {
          bgColor = "#fdf5ee";
          borderColor = "#944920";
          fontColor = "#522510";
        } else if (node.group === "neurona") {
          bgColor = "#f0f6f9";
          borderColor = "#205b76";
          fontColor = "#0c3b52";
        } else if (node.group === "meninges") {
          bgColor = "#faf3e6";
          borderColor = "#6d4304";
          fontColor = "#451a03";
        }
        ...
        return {
          id: node.id,
          label: node.label,
          title: `${node.label} - ${node.sublabel || node.category}`,
  ...
  const handleSearchNode = (term: string) => {
    ...
    const match = neuroNodes.find(n => n.label.toLowerCase().includes(clean) || (n.sublabel && n.sublabel.toLowerCase().includes(clean)));
    if (match) {
  ...
  const renderCategoryIcon = () => {
    if (selectedNode.category === "Planimetría") {
      return <Compass className="w-5 h-5 text-[#2b553c]" />;
    } else if (selectedNode.category === "Cisuras y Lóbulos") {
      return <Brain className="w-5 h-5 text-[#944920]" />;
    } else if (selectedNode.category === "Neurona y Matriz") {
      return <Microscope className="w-5 h-5 text-[#205b76]" />;
    } else if (selectedNode.category === "Meninges y LCR") {
      return <ShieldCheck className="w-5 h-5 text-[#6d4304]" />;
    }
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-3 bg-[#fbf8f5] p-3.5 rounded-xl border border-[#d4cbc2]/60">
        <div>
          ...
          <p className="text-xs text-[#49423c] mt-0.5">
            Explora las conexiones entre planimetría, cisuras, neuronas, matriz SNC/SNP, meninges y LCR.
          </p>
        </div>
        ...
        <div className="lg:col-span-8 bg-[#fbf8f3] rounded-2xl border-2 border-[#e7ded3] p-2 relative shadow-parchment overflow-hidden flex flex-col">
          ...
          <div className="p-2 border-b border-[#d4cbc2]/40 flex flex-wrap items-center justify-between gap-2 z-10 bg-[#fbf8f3]/95 backdrop-blur-xs">
            <div className="flex items-center gap-1.5 flex-wrap text-xs">
              ...
              <button 
                onClick={() => handleFilter('Planimetría')}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition ${activeFilter === 'Planimetría' ? 'bg-[#2b553c] text-white' : 'bg-[#f5efe9] hover:bg-[#ebe3dc] text-[#49423c]'}`}
              >
                Planimetría
              </button>
              <button 
                onClick={() => handleFilter('Cisuras y Lóbulos')}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition ${activeFilter === 'Cisuras y Lóbulos' ? 'bg-[#944920] text-white' : 'bg-[#f5efe9] hover:bg-[#ebe3dc] text-[#49423c]'}`}
              >
                Cisuras y Lóbulos
              </button>
              <button 
                onClick={() => handleFilter('Neurona y Matriz')}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition ${activeFilter === 'Neurona y Matriz' ? 'bg-[#205b76] text-white' : 'bg-[#f5efe9] hover:bg-[#ebe3dc] text-[#49423c]'}`}
              >
                Neurona y Matriz
              </button>
              <button 
                onClick={() => handleFilter('Meninges y LCR')}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition ${activeFilter === 'Meninges y LCR' ? 'bg-[#6d4304] text-white' : 'bg-[#f5efe9] hover:bg-[#ebe3dc] text-[#49423c]'}`}
              >
                Meninges y LCR
              </button>
            </div>
          ...
          <div className="mt-2.5 pt-2 border-t border-[#e8dfd4] px-3 py-1 flex flex-wrap items-center justify-between gap-3 text-xs text-[#49423c] font-serif">
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-full bg-[#2b553c] inline-block shadow-xs"></span>
                <span>Planimetría</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-full bg-[#944920] inline-block shadow-xs"></span>
                <span>Cisuras y Lóbulos</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-full bg-[#205b76] inline-block shadow-xs"></span>
                <span>Neurona & Matriz SG/SB</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-full bg-[#6d4304] inline-block shadow-xs"></span>
                <span>Meninges y LCR</span>
              </div>
            </div>
            <div className="text-[11px] text-[#787169] italic flex items-center gap-1 font-sans">
            ...
            <div>
              <div className="flex items-start justify-between gap-2 border-b border-[#d4cbc2]/40 pb-3">
                <div>
                  ...
                  <h3 className="text-lg font-bold text-[#292420] mt-1 font-serif">
                    {selectedNode.label}
                  </h3>
              ...
              {/* Description */}
              <div className="py-3">
                <h4 className="text-xs font-bold text-[#787169] uppercase tracking-wider mb-1">Definición de clase:</h4>
